...
<div style="width:95%;margin:auto;">
    <div class="block block-rounded block-bordered">
        ...
        <div class="block-content mb-3">
            <form action="{{route('mperson.jobdescriptions_personnel_estimate6_update')}}" method="post" autocomplete="false">
            @csrf()
            <input type="hidden" name="id" value="{{$job_person_list[0]->IWJOB_PERSON_ID}}">
                <div class="container-fluid fs-14 mb-4">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="d-flex">
                                <div style="width:140px">อัพเดตล่าสุด</div>
                    ...
                    <table width="100%" border="1px" >
                        ...
                        <tbody class="bg-sl-header text-center fs-10 padding-custom">
                            <?php
                                ...
                                $multiply_total = 0;
                            ?>
                            @if(!empty($job_person_list[0]->IWJOB_PERSON_LIST_ID))
                            ...
                            <tr>
                                ...
                                <td class="bg-white p-0">
                                    <input name="performance_10[]" id="performance_10_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_10}}" step="0.01" min="0" max="100">
                                </td>
                                <td class="bg-white p-0">
                                    <input name="performance_11[]" id="performance_11_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_11}}" step="0.01" min="0" max="100">
                                </td>
                                <td class="bg-white p-0">
                                    <input name="performance_12[]" id="performance_12_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_12}}" step="0.01" min="0" max="100">
                                </td>
                                <td class="bg-white p-0">
                                    <input name="performance_1[]" id="performance_1_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_1}}" step="0.01" min="0" max="100">
                                </td>
                                <td class="bg-white p-0">
                                    <input name="performance_2[]" id="performance_2_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_2}}" step="0.01" min="0" max="100">
                                </td>
                                <td class="bg-white p-0">
                                    <input name="performance_3[]" id="performance_3_{{$number}}" class="fs-12 text-center border-none" type="number" style="width:100%" value="{{$row->IWJPL_PERFORMANCE_3}}" step="0.01" min="0" max="100">
                                </td>
                            </tr>
                            <?php
                            ...
                            ?>
                            @endforeach
                            <input type="hidden" id="check_rowkpi" name="check_rowkpi" value="1">
                            @else
                            <input type="hidden" id="check_rowkpi" name="check_rowkpi" value="0">
                            @endif
                        </tbody>
                ...
                <div class="d-flex justify-content-end">
                    <button onclick="return confirm('ต้องการยืนยันการประเมิน 6 เดือนแรกจริงหรือไม่')" type="submit" class="btn btn-info mr-2"><i class="fa fa-save mr-2"></i>ยืนยันการประเมิน</button>
...
<script>
    $('form').on('submit',function () {
        if($('#check_rowkpi').val() == 1){
            return true; 
        }else{
            console.log($('#check_rowkpi').val());
            Swal.fire("ไม่สามารถบันทึกได้","ไม่พบ kpi ภายใน job description",'error')
            return false; 
        }
    })
